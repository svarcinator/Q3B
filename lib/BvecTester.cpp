...
bool BvecTester::testBvecSize(size_t size, const Approximated<cudd::Bvec>& approxResult) {
    auto bvecSize = approxResult.value.m_bitvec.size();
    auto same =  (size == bvecSize);
    assert(same);
    return same;
}
