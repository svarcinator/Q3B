...
void print_usage()
{
    ...
    std::cout << "Usage: q3b [options] file.smt2" << std::endl
              << std::endl;
    ...
    while ((opt = getopt_long(argc, argv, "a:m:b:p:g:r:i:e:s:l:c:C:f:v:hV", long_options, &long_index)) != -1) {
        ...
        case 'm': {
            ...
            if (optionString == "variables")
                ...
                std::cout << "Invalid interval recomputation type" << std::endl;
                print_usage();
                exit(1);
            }
            break;
        ...
        case 'e': {
            ...
            if (optionString == "demanding")
                config.interval_recomputation = DEMANDING_OPS;
            else if (optionString == "all")
                config.interval_recomputation = ALL_OPS;
            else if (optionString == "none")
                config.interval_recomputation = NO_OPS;
            else {
